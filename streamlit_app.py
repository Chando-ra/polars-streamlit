...
# --- データ読み込みとキャッシュ ---
@st.cache_data
def load_data(selected_files: list[Path]) -> pl.DataFrame:
    """
    指定されたParquetファイルを読み込み、結合する。
    """
    ...
    try:
        df_list = [pl.read_parquet(file) for file in selected_files]
...
if selected_files_paths:
    df = load_data(selected_files_paths)
else:
    df = pl.DataFrame()
...
if not df.is_empty():
    ...
    # --- メインコンテンツ ---
    if agg_col and not filtered_df.is_empty():
        st.header(f"`{agg_col}`別 サマリー")

        top_by_count = (
            filtered_df.group_by(agg_col)
        ...
        with col1:
            ...
            st.dataframe(top_by_count.to_pandas())
        with col2:
            st.subheader("EVENT_VALUE合計 トップ100")
            st.dataframe(top_by_value.to_pandas())

        st.header("月次トレンド分析")

        # --- グラフ1: レコード数 ---
        top_n_by_count_cats = top_by_count.head(top_n)[agg_col].to_list()
        df_for_count_chart = filtered_df.with_columns(
        ...
        df_for_value_chart = filtered_df.with_columns(
